...
public class ViewModelNoticeHandler implements Runnable {
	...
	public ViewModelNoticeHandler(QTableView view, FSObjectTableModel model, BlockingQueue<ViewModelNotice> readInQueue) {
		super();
		this.view = view;
		this.model = model;
	...
	public ViewModelNoticeHandler(QTableView view, FSObjectTableModel model, BlockingQueue<ViewModelNotice> readInQueue, ExecutorService executor) {
		super();
		this.view = view;
		this.model = model;
		this.readInQueue = readInQueue;
				...
				if (HyperSQLManager.match(notice.getFSObject().getName(), model.getCriterion())) {
					...
					this.executor.submit(new Runnable() {
						...
						public void run() {
							...
							// QRect boundingRect =
							// view.visibleRegion().boundingRect();
							// int firstVisibleRow =
							// view.rowAt(boundingRect.top());
							// int lastVisibleRow =
							// view.rowAt(boundingRect.bottom());
							// LOG.debug("firstVisibleRow:{}, lastVisibleRow:{}",
							// firstVisibleRow, lastVisibleRow);

							// TODO:
							// 当监控到文件变化时,只有当变化的文件在当前的可视区域中时,才需要从cache中删除条目,并通知更新
