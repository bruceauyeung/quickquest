...
public class PreferencesDialog extends QDialog {
	...
	public PreferencesDialog(Main parent) {
		...
		cancelBtn.setSizePolicy(QSizePolicy.Policy.Fixed,
				QSizePolicy.Policy.Fixed);

		QPushButton applyBtn = new QPushButton("Apply", this);
		applyBtn.clicked.connect(this, "slotApplyBtnHandler()");
		applyBtn.setSizePolicy(QSizePolicy.Policy.Fixed,
				QSizePolicy.Policy.Fixed);
		...
		btnLayout.setAlignment(new Qt.Alignment(Qt.AlignmentFlag.AlignRight));
	...
	private void loadPreferences() {
		...
		directoryTab.getDbLocLineEdit()
				.setText(
						prefs.getDirectoryTab().getDatabaseLocation()
								.getAbsolutePath());

		for (MonitoredDirectory moniDir : prefs.getDirectoryTab()
				.getMonitoredDirectories()) {
			directoryTab.addMonitorDir(
					moniDir.getDirectory().getAbsolutePath(),
					moniDir.isInSearchPath(), moniDir.isChangesMonitored());
		}
	...
	private void slotApplyBtnHandler() {
		...
		MonitoredDirectoryDiff diff = saveDirectoryTab();
		QuickQuest.savePreferencesToDisk();

		for (MonitoredDirectory md : diff.getAdds()) {
			addMonitoredDirectory(md);
		}
		for (MonitoredDirectory md : diff.getMods()) {
			modMonitoredDirectory(md);
		}
		for (MonitoredDirectory md : diff.getDels()) {
			delMonitoredDirectory(md);
		}
	}
	...
	private class DirectoryTab extends QWidget {
		...
		public DirectoryTab(QWidget parent) {
			...
			Preferences prefs = QuickQuest.getPreferences();
			dbLocLineEdit = new QLineEdit(prefs.getDirectoryTab()
					.getDatabaseLocation().getAbsolutePath(), this);
			...
			monitorDirListWidget.currentRowChanged.connect(this,
					"slotCurMonDirChanged(int)");
			...
			final QPushButton delMonDirBtn = new QPushButton(tr("&Delete"),
					this);
		...
		private QGroupBox createMoniDirSettingsPage(String dirPath,
				boolean isInSearchPath, boolean changesMonitored) {
			return new MoniDirSettingsPage(tr("Settings for Directory ") + "("
					+ dirPath + ")", isInSearchPath, changesMonitored);
		}

		private void slotAddMonitorDir() {
			String directory = QFileDialog.getExistingDirectory(this,
					tr("Select a directory to be monitored"),
					QDir.currentPath());
			if (!directory.equals("")) {
		...
		private void addMonitorDir(String directory, boolean isInSearchPath,
				boolean changesMonitored) {
			QListWidgetItem monitorDirItem = new QListWidgetItem(
					monitorDirListWidget);
			...
			settingsPagesWidget.addWidget(createMoniDirSettingsPage(directory,
					isInSearchPath, changesMonitored));
			settingsPagesWidget.setCurrentIndex(monitorDirListWidget
					.currentRow());
		}

		private void slotDelMonitorDir() {
			final QListWidgetItem currentItem = monitorDirListWidget
					.currentItem();
			monitorDirListWidget.removeItemWidget(currentItem);

			// dispose must be called after calling removeItemWidget method, or
			// that item will not be removed.
			currentItem.dispose();
		}
	...
	private class MoniDirSettingsPage extends QGroupBox {
		...
		public MoniDirSettingsPage(String title, boolean isInSearchPath,
				boolean changesMonitored) {
			super(title);
	...
	private class MonitoredDirectoryDiff {
		private List<MonitoredDirectory> adds = Lists.newArrayList();
		private List<MonitoredDirectory> mods = Lists.newArrayList();
		private List<MonitoredDirectory> dels = Lists.newArrayList();

		public List<MonitoredDirectory> getAdds() {
			return adds;
		}

		public List<MonitoredDirectory> getMods() {
			return mods;
		}

		public List<MonitoredDirectory> getDels() {
			return dels;
		}

		public void add(MonitoredDirectory md) {
			if (!adds.contains(md)) {
		...
		public void mod(MonitoredDirectory md) {
			if (!mods.contains(md)) {
				mods.add(md);
			}
		...
		public void del(MonitoredDirectory md) {
			if (!dels.contains(md)) {
	...
	private MonitoredDirectoryDiff saveDirectoryTab() {
		MonitoredDirectoryDiff diff = new MonitoredDirectoryDiff();
		Preferences prefs = QuickQuest.getPreferences();

		// save database location, if this location is changed, hsqldb files
		// will be moved to new location after reboot.
		String databaseLocation = directoryTab.getDbLocLineEdit().text();
		final File newDbLocFile = new File(databaseLocation);
		final net.ubuntudaily.quickquest.preferences.DirectoryTab dirTab = prefs
				.getDirectoryTab();
		...
		for (int i = 0; i < moniDirListWidget.count(); i++) {
			...
			// if the directory does not exist, there is no need to search or
			// monitor it
			...
			if (settingsPage instanceof MoniDirSettingsPage) {
				...
				isInSearchPath = moniDirSettingsPage.getInSearchPathCtrl()
						.isChecked();
				changesMonitored = moniDirSettingsPage
						.getChangesMonitoredCtrl().isChecked();

				MonitoredDirectory existed = dirTab
						.getMonitoredDirectory(new File(moniDir));
				if (existed != null) {
					dels.remove(existed);
					...
					final MonitoredDirectory addedMoniDir = new MonitoredDirectory(
							new File(moniDir), isInSearchPath, changesMonitored);
					dirTab.getMonitoredDirectories().add(addedMoniDir);
					diff.add(addedMoniDir);

				}
				
				for(MonitoredDirectory md : dels){
					diff.del(md);
				}
			}
		}
	...
	private void addMonitoredDirectory(final MonitoredDirectory addedMoniDir) {
		if (this.parentWidget() instanceof Main) {
			Main main = (Main) this.parentWidget();
			main.addMonitoredDirectory(addedMoniDir);
		}
	}
	private void delMonitoredDirectory(final MonitoredDirectory addedMoniDir) {
		if (this.parentWidget() instanceof Main) {
			Main main = (Main) this.parentWidget();
			main.delMonitoredDirectory(addedMoniDir);
		}
	}	
	private void modMonitoredDirectory(final MonitoredDirectory addedMoniDir) {
		if (this.parentWidget() instanceof Main) {
			Main main = (Main) this.parentWidget();
			main.modMonitoredDirectory(addedMoniDir);
		}
	}		
}
